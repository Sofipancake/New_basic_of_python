...
#Обработать все элементы списка и вернуть в качестве результата
#список, состоящий из фраз вида:
...
#Можно ли при этом не создавать новый список? Можно, если изменить список имен

def convert_name_extract(list_in: list) -> list:
    #    """Извлекает имена из элементов и формирует список приветствий."""
    list_out = []                                                             #убрать строку
    for i in range(len(list_in)):
        list_out.append(f'Привет, {list_in[i].split()[-1].title()}!')          #list_in[i] = (f'Привет, {list_in[i].split()[-1].title()}!')
    return (list_out)                                                          #return (list_in)
